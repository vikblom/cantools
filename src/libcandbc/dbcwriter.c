...
static void mux_info_write(FILE *out, signal_t *signal)
{
    ...
    case m_multiplexor:
        ...
        break;
    case m_multiplexed:
        fprintf(out," m%u",signal->mux_value);
        break;
    case m_signal:
    default:
        break;
...
{
    fprintf(out, "BO_ %u %s: %u %s" NEWLINE,
            message->id,
            message->name,
            message->len,
            message->sender);
...
static void message_comment_write(FILE *out, message_t *message)
{
    if(message->comment != NULL) {
        fprintf(out, "CM_ BO_ %u \"%s\";" NEWLINE, message->id, message->comment);
    }
}
...
static void message_attribute_list_write(FILE *out, message_list_t *message_list)
{
    PLIST_ITER(message_list) {
        ...
        sprintf(target, "BO_ %u", message_list->message->id);
    ...
    attribute_rel_t *attribute_rel)
{
    ...
    fprintf(out, " SG_ %u ", attribute_rel->message->id);
...
static void signal_val_map_write(FILE *out, message_list_t *message_list)
{
    PLIST_ITER(message_list) {
        ...
        PLIST_ITER(signal_list) {
            ...
            if(val_map != NULL) {
                fprintf(out, "VAL_ %u %s ", message->id, signal->name);
    ...
    message_list_t *message_list)
{
    PLIST_ITER(message_list) {
        ...
        if(message->transmitter_list != NULL) {
            fprintf(out, "BO_TX_BU_ %u : ", message->id);
    ...
    PLIST_ITER(message_list) {
        ...
        PLIST_ITER(signal_list) {
            ...
            if(signal->signal_val_type == svt_float) {
                fprintf(out, "SIG_VALTYPE_ %u %s : 1;", message->id, signal->name);
            } else if(signal->signal_val_type == svt_double ) {
                fprintf(out, "SIG_VALTYPE_ %u %s : 2;", message->id, signal->name);
            }
        }
    }
}
